...
# --- USER INPUT ---
wrf_date_time = datetime(2022,11,19,2,52,42)
domain = 2
SIMULATION = 1 # If comparing runs

notes = f"IOP2 Third Sounding Comparison\nAttempt {SIMULATION}\nDomain {domain}" # To be added to whitespace in figure

path = f"/data2/white/WRF_OUTPUTS/PROJ_LEE/ELEC_IOP_2/ATTEMPT_{SIMULATION}/"
sounding_path = "/data2/white/DATA/PROJ_LEE/IOP_2/SOUNDINGS/MW41_output_textFormat_raw_20221119_025242.txt"
...
lat_lon = [43.88621939364891, -76.1572628502298] # Coordinates for Simulated Sounding, IOP 2 NSSL: 43.88621939364891, -76.1572628502298 
...
df = pd.read_csv(sounding_path, sep=r'\s+', header=6, names=column_names)
...
p = p[:index_of_pop]
temp  = df['Temperature'].values[:index_of_pop] * units.degC
wspd = df['Wind_Speed'].values[:index_of_pop] * units.meter / units.second
wdir = df['Wind_Direction'].values[:index_of_pop] * units.deg
rh = df['Relative_Humidity'].values[:index_of_pop] * units.percent
...
plt.suptitle(notes)
plt.savefig(savepath+f"NSSL_skewTA{SIMULATION}D{domain}{file_date}.png")
...
ax_obs.set_title(f"Closest Observed Scan")
