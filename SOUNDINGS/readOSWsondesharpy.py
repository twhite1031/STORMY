...
# --- USER INPUT ---
wrf_date_time = datetime(2022,11,17,9,00,00)
domain = 2
...
sounding_path = "/data2/white/DATA/PROJ_LEE/IOP_2/SOUNDINGS/OSW20221117_09Z_sharppy.txt" 
...
column_names = ['Pressure', 'Height', 'Temperature', 'Dewpoint', 'Wind_Direction', 'Wind_Speed']

df = pd.read_csv(sounding_path,skiprows=11,skipfooter=2,sep=',', header=None, names=column_names)

# Convert the necessary columns to appropriate units
p  = df['Pressure'].values * units.hPa
temp  = df['Temperature'].values * units.degC
wspd = df['Wind_Speed'].values * units.knots
wdir = df['Wind_Direction'].values * units.deg
dewtemp = df['Dewpoint'].values * units.degC
...
# Mask out levels with pressure <= 100 hPa
valid = p > 300 * units.hPa

skew.plot(p[valid], temp[valid], 'r')
skew.plot(p[valid], dewtemp[valid], 'g')

# Apply slicing after filtering
skew.plot_barbs(p[valid][::20], u[valid][::20], v[valid][::20])
...
u1 = wrf.getvar(wrfin,"ua",timeidx=matched_timeidx, units='kts')
v1 = wrf.getvar(wrfin,"va",timeidx=matched_timeidx,units='kts')
