...
"""
A plot used to outline max reflectivity values of 20 dBZ (can be changed) as a base
...
def generate_frame(args):
    ...
    # Set the map bounds
        ax_N.set_xlim(cartopy_xlim(mdbz_N))
        ax_N.set_ylim(cartopy_ylim(mdbz_N))
    ...
    # Set the map bounds
        ax_F.set_xlim(cartopy_xlim(mdbz_F))
        ax_F.set_ylim(cartopy_ylim(mdbz_F))
    ...
    # Add the gridlines
      ...
      # Read in cmap and map contours
        ...
        mdbz_N_contour = ax_N.contour(to_np(lons), to_np(lats), mdbz_N,levels=[20],cmap=nwscmap, transform=crs.PlateCarree())
        mdbz_F_contour = ax_F.contour(to_np(lons), to_np(lats), mdbz_F,levels=[20],cmap=nwscmap, transform=crs.PlateCarree())
    ...
    # Set titles, get readable format from WRF time
        ...
        ax_N.set_title(f"Normal Simulation at " + str(time_object_adjusted),fontsize=12,fontweight='bold')
        ax_F.set_title(f"Flat Simulation at " + str(time_object_adjusted),fontsize=12,fontweight='bold')
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=20) as executor: # Use ProcessPoolExecutor for CPU-bound tasks
