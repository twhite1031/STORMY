...
def generate_frame(args):
    ...
    # Add the gridlines
      ...
      # Read in cmap and map contours
        ...
        mdbz_N_contour = ax_N.contourf(to_np(lons), to_np(lats), mdbz_N,levels=levels,cmap=nwscmap, transform=crs.PlateCarree())
        mdbz_F_contour = ax_F.contourf(to_np(lons), to_np(lats), mdbz_F,levels=levels,cmap=nwscmap, transform=crs.PlateCarree())
        
        # Create a shared colorbar to the right of both subplots
        cbar = fig.colorbar(mdbz_N_contour, ax=[ax_N, ax_F], orientation='vertical', shrink=0.5, pad=0.02)
    ...
    # Set titles, get readable format from WRF time
        ...
        ax_N.set_title(f"Normal Simulation Composite Reflectivity (dBZ) at " + str(time_object_adjusted),fontsize=18,fontweight='bold')
        ax_F.set_title(f"Flat Simulation Compostite Reflectivity (dBZ) at " + str(time_object_adjusted),fontsize=18,fontweight='bold')
