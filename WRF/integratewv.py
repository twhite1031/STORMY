...
def generate_frame(file_path_N, file_path_F, timeidx,time):
    try:
        ...
        # Read data from matched WRF files for each model run. Multiplied by 1000 to convert from kg/kg to g/kg
        with Dataset(file_path_N) as wrfin:
        ...
        # Apply horizontal mask for each variable in each model run (only keep data in the region of interest)
        ...
        # Compute air density (ρ) using the ideal gas law for each model run
        ...
        # Compute vertical grid spacing (dz) at each level for each model run
        ...
        dx = 3000  # Grid spacing in meters 
        ...
        # Compute the mass of water vapor, snow, ice, and graupel in the region for each model run
        ...
        # Append the date data to a list for plotting later on
        datetime_array.append(time.strftime("%d %H:%M"))

        WV_array_N.append(WV_mass_N / 1000) # Scaling back to kg for plotting
        WV_array_F.append(WV_mass_F / 1000) # Scaling back to kg for plotting
...
if __name__ == "__main__":
    ...
    # Create a list of tasks to process each file pair
    ...
    plt.title("Change in Mass of Hydrometeors (g) (WV: kg)", fontsize=22,fontweight='bold')
    plt.xticks(datetime_array[::3],rotation=45)
    plt.legend()
    plt.grid()

    # Format the time for a filename (no spaces/colons), show and save figure
    starttime_str, endtime_str = start_time.strftime("%Y-%m-%d_%H-%M-%S"), end_time.strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"integratewv_{start_time}_{end_time}.png"
