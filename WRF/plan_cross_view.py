...
# Read data from the matched WRF file, vapor multiplied by 1000 to convert from kg/kg to g/kg
with Dataset(matched_file) as ds:
    ...
    vapor = getvar(ds, "QVAPOR", timeidx=matched_timeidx) * 1000
...
# Make a copy of the reflectivity cross to fix data.
...
# Same processes for water vapor
...
vapor_levels = np.arange(0,3.01,.3)
...
# Plot the cross section line on the reflectivity plot
ax_plan.plot([cross_start.lon, cross_end.lon],[cross_start.lat, cross_end.lat], color="brown", marker="o",transform=crs.PlateCarree())

# "Plot" the filled contour of water vapor for the color bar creation, but is removed later
...
# Get vertical indices for vertical velocity cross section
z_cross = to_np(w_cross.coords["vertical"])

# Create a zero horizontal component to highlight vertical velocity
...
# Fill in the terrain on the cross section
ht_fill_vapor = ax_cross.fill_between(xs, 0, to_np(ter_line),facecolor="saddlebrown")
...
ax_plan.set_title(f"Plan view of Composite Reflectivity (dBZ) at {matched_time}", fontsize=14, fontweight='bold')
